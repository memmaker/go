...
type Application struct {
	...
	unknownEventCapture func(event tcell.Event) tcell.Event
...
func (a *Application) Run() error {
	...
	handle := func(event tcell.Event) {
		a.RLock()
		p := a.focus
		inputCapture := a.inputCapture
		unknownEventCapture := a.unknownEventCapture
		screen := a.screen
		a.RUnlock()
		...
		default:
			// Ignore.
			if unknownEventCapture != nil {
				event = unknownEventCapture(event)
				if event == nil {
					a.draw()
...
func (a *Application) SetUnknownEventCapture(eventCapture func(event tcell.Event) tcell.Event) {
	a.unknownEventCapture = eventCapture
