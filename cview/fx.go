...
func lightScreenLocation(screen tcell.Screen, screenCopy []StyledRune, x int, y int, amount int32, w int, h int) bool {
    ...
    originalFG, originalBG, orgAttr := originalStyle.Decompose()
    ...
    var drawStyle tcell.Style
    if orgAttr&tcell.AttrReverse != 0 {
        drawStyle = originalStyle.Background(tcell.NewRGBColor(int32(newFgRed), int32(newFgGreen), int32(newFgBlue))).Foreground(tcell.NewRGBColor(int32(newBgRed), int32(newBgGreen), int32(newBgBlue)))
    } else {
        drawStyle = originalStyle.Background(tcell.NewRGBColor(int32(newBgRed), int32(newBgGreen), int32(newBgBlue))).Foreground(tcell.NewRGBColor(int32(newFgRed), int32(newFgGreen), int32(newFgBlue)))
    }
