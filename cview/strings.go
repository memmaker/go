...
func RightPad(s string, pLen int) string {
    return s + strings.Repeat(" ", pLen-TaggedStringWidth(s))
}

func LeftPad(s string, pLen int) string {
    return strings.Repeat(" ", pLen-TaggedStringWidth(s)) + s
}
...
func TableLayoutLastRight(tableData []TableRow) []string {
    alignments := make([]int, len(tableData[0].Columns))
...
func TableLayout(tableData []TableRow, alignments []int) []string {
    colWidths := make([]int, len(tableData[0].Columns))

    for _, row := range tableData {
        for i, col := range row.Columns {
            colWidth := TaggedStringWidth(col)
