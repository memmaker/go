...
func (h HDRColor) Lerp(otherColor HDRColor, percent float64) HDRColor {
    return HDRColor{
        R: Lerp(h.R, otherColor.RValue(), percent),
        G: Lerp(h.G, otherColor.GValue(), percent),
        B: Lerp(h.B, otherColor.BValue(), percent),
        A: Lerp(h.A, otherColor.AValue(), percent),
    }
}
...
func (h HDRColor) Desaturate() HDRColor {
    luminance := Clamp(h.RelativeLuminance(), 0.0, 1.0)
    //luminance := Clamp(R.Lightness(), 0.0, 1.0)
    return HDRColor{luminance, luminance, luminance, 1.0}
}
...
func (h HDRColor) ReinhardToneMapping() (r, g, b, a uint32) {
    lightness := h.Lightness()
    scale := 0xffff * math.Sqrt(lightness)
    // vec3 mapped = hdrColor / (hdrColor + vec3(1.0));
    //gamma := 2.2
    mappedR := (h.R * scale) / (h.R + 1.0)
    //mappedR = math.Pow(mappedR, 1.0/gamma)
    r = uint32(mappedR)
    mappedG := (h.G * scale) / (h.G + 1.0)
    //mappedG = math.Pow(mappedG, 1.0/gamma)
    g = uint32(mappedG)
    mappedB := (h.B * scale) / (h.B + 1.0)
    //mappedB = math.Pow(mappedB, 1.0/gamma)
    b = uint32(mappedB)
    a = uint32(0xffff)
    return r, g, b, a
}
func (h HDRColor) LightnessScaledToneMapping() (r, g, b, a uint32) {
    lightness := h.Luminance()
    scale := 0xffff * math.Sqrt(lightness)
    r = uint32(h.R * scale)
    g = uint32(h.G * scale)
    b = uint32(h.B * scale)
...
func (h HDRColor) Lightness() float64 {
    y := h.RelativeLuminance()
    var result float64
    if y <= (216.0 / 24389) {
        result = y * (24389.0 / 27)
    } else {
