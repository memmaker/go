...
func (p *LineBrush) DraggedOver(pos geometry.Point) []geometry.Point {
	return geometry.LineOfSight(p.startPos, pos, func(p geometry.Point) bool {
		return true
	})
}
...
func (r *RectangleBrush) outlinedRect() []geometry.Point {
	outline := NewPointSet()
	for x := r.drawRect.Min.X; x <= r.drawRect.Max.X; x++ {
		outline.Add(geometry.Point{X: x, Y: r.drawRect.Min.Y})
		outline.Add(geometry.Point{X: x, Y: r.drawRect.Max.Y})
	}
	for y := r.drawRect.Min.Y; y <= r.drawRect.Max.Y; y++ {
		outline.Add(geometry.Point{X: r.drawRect.Min.X, Y: y})
		outline.Add(geometry.Point{X: r.drawRect.Max.X, Y: y})
	}
	return outline.ToSlice()
}
