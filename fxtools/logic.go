...
func (a Arguments) GetBool(index int) bool {
	if index >= len(a) || index < 0 {
		return false
	}
	parseBool, _ := strconv.ParseBool(a[index])
	return parseBool
}
...
func (a Arguments) GetUint64(index int) uint64 {
	if index >= len(a) || index < 0 {
		return 0
	}
	result, err := strconv.ParseUint(a[index], 10, 64)
	if err != nil {
		return 0
	}
	return result
